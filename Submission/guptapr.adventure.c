...
/*
Compile: gcc -std=c89 guptapr.adventure.c -o guptapr.adventure -lpthread
Execute: ./guptapr.adventure
Return: Update the Maze and play game
Example Game:
$ ./guptapr.adventure 
CURRENT LOCATION: XYZZY
POSSIBLE CONNECTIONS: Ezio, Drake, PLOVER.
WHERE TO? >time

 5:15pm, Friday May  1, 2020

WHERE TO? >time

 5:16pm, Friday May  1, 2020
...
CURRENT LOCATION: Ezio
POSSIBLE CONNECTIONS: XYZZY, PLOVER, Dungeon, Zork.
WHERE TO? >PLOVER

CURRENT LOCATION: PLOVER
POSSIBLE CONNECTIONS: Zork, Dungeon, Ezio, Drake, XYZZY.
WHERE TO? >time
...
CURRENT LOCATION: Drake
POSSIBLE CONNECTIONS: PLUGH, Dungeon, PLOVER, XYZZY.
WHERE TO? >Dungeon 
...
{
    /*While loop for infinite calls from user to check time*/
    while (1)
    {
        ...
        /*Allow mutex to lock run_thread_b now*/
        run_thread_b = 0;
        pthread_mutex_unlock(&run_lock_b);
        /*Save the time to currentTime*/
        ...
        assert(strftime(current_time, sizeof(current_time), "%l:%M%P, %A %B %e, %Y", tm));
        fprintf(fptr, "%s\n", current_time);
...
/*Read time from currentTime and show on display*/
void printTime()
{
    char readTime[255];
...
void *mainGame(void *arguments)
{
    ...
    init_rooms();
    /*Identify last saved folder*/
    folder = last_saved();

    /*Read files to populate Room Name and Room Type*/
    dir = opendir(folder);
    if (dir == NULL)
    {
    ...
    closedir(dir);
    /*Reopen File for updating connections*/
    i = 0;
    dir = opendir(folder);
    ...
    closedir(dir);
    /*Update previous location to check user input*/
    strcpy(PREVIOUS_LOCATION, CURRENT_LOCATION->title);
    /*Repeat for user input*/
    while (1)
    {
        loop_in = 0;
        /*Exit if reached end location*/
        ...
        /*Loop till user stop requesting for time*/
